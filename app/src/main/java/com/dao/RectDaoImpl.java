...
public class RectDaoImpl {
    ...
    public List<SpeakingRect> getAllRects(){
        ...
        for (SpeakingRect rect : allRectsFromDB){
            allRectsOfAPicResult.add(new SpeakingRect(rect.getId(),rect.getPicName(),rect.getPoint()));
        }
    ...
    public SpeakingRect getRect(String picName, Point point){
        ...
        return new SpeakingRect(rect.getId(),rect.getPicName(),rect.getPoint());//rect with all values filled from the constructor
    }

    public void insertRect(String picName, Point point){ // id is autogenerated - pass 0 as not set
        if(picName != null){
            appDb.rectDao().insertRect(new SpeakingRect(0,picName,point));//call constructor for default values
        }
    }
